<script lang="ts">
    ...
    let scrollYProgressVelocity = $state(0);
    ...
    $effect(() => {
        if (typeof window !== "undefined") {
            let newScrollYProgress =
                window.scrollY / document.body.offsetHeight;
            scrollYProgressVelocity = newScrollYProgress - scrollYProgress;
            scrollYProgress = newScrollYProgress;
...
<div
    bind:this={ref}
    class={cn("relative mx-auto h-full w-full max-w-4xl", className)}
>
        ...
        >
            <div
                ...
                style="background-color: {scrollYProgress > 0
                    ? 'white'
                    : 'var(--color-emerald-500)'}; border-color: {scrollYProgress >
                0
                    ? 'white'
